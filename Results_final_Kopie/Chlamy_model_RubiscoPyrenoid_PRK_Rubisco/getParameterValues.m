...
for SAMPLE = idx
    ...
    x_lb = log(ones(size(alpha,2),1)*1e-10);
    ...
    x_lb([71:73 75:109 111:114 117:128]) = log(1e-50*ones(length([71:73 75:109 111:114 117:128]),1));
    x_ub([71:73 75:109 111:114 117:128]) = log(1e-50*ones(length([71:73 75:109 111:114 117:128]),1)); % pyrenoid metabolites
     
    noNANk = k_red_id_s(~isnan(k_red_id_s));
    % bounds on kcat values
    k_lb = log(1e-50*ones(size(alpha,1),1));
    k_lb(noNANk([1 3 4 5 6 7 8 9 11])) = log(kcat([1 3 4 5 6 7 8 9 11]));
